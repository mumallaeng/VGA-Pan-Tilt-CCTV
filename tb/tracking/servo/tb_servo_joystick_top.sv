...
module tb_servo_joystick_top;
    ...
    initial begin
        ...
        if (dut.u_servo.pan_angle !== 8'd90 ||
            ...
            errors = errors + 1;
            $error("reset failed: pan=%0d tilt=%0d", dut.u_servo.pan_angle,
                   dut.u_servo.tilt_angle);
        end
        ...
        #1;
        if (dut.joy_dx <= 0 || dut.joy_dy >= 0 || $unsigned(
                dut.u_servo.pan_angle
            ) <= 90 || $unsigned(
                dut.u_servo.tilt_angle
            ) >= 90) begin
            errors = errors + 1;
            $error(
                "real joystick-to-servo forward path failed: x=%0d y=%0d pan=%0d tilt=%0d",
                dut.joy_dx, dut.joy_dy, dut.u_servo.pan_angle,
                dut.u_servo.tilt_angle);
        end
        ...
        if (dut.joy_dx >= 0 || dut.joy_dy <= 0 || $unsigned(
            ...
            errors = errors + 1;
            $error(
                "real joystick RTL reverse mapping failed: x=%0d y=%0d pan=%0d tilt=%0d",
                dut.joy_dx, dut.joy_dy, dut.u_servo.pan_angle,
                dut.u_servo.tilt_angle);
        end
        ...
        else $fatal(1, "%0d real joystick-to-servo tests failed", errors);
        $finish;
    end
endmodule
