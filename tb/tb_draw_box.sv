...
//   PHASE 4 - full 320x240 source sweep: every pixel compared against an
//             independent model, and the lit-pixel count compared against
//             the analytic perimeter area for both THICKNESS values
...
module tb_draw_box ();
    ...
    // ------------------------------------------------------------
    // Sweep the whole 320x240 source image (one screen pixel per source
    // pixel), comparing both DUTs against the model and counting the lit
    // pixels so the border area can be checked analytically.
    // ------------------------------------------------------------
    task automatic sweep_frame(input string name, input int exp_lit, input int exp_lit_t1);
        ...
        begin
            ...
            for (sy = 0; sy < IMG_H; sy++) begin
                for (sx = 0; sx < IMG_W; sx++) begin
                    @(posedge clk);
                    vga_x <= 10'(2 * sx);
                    vga_y <= 10'(2 * sy);
                    #1;

                    model_box    = ref_box_en(10'(2 * sx), 10'(2 * sy), THICKNESS);
                    model_box_t1 = ref_box_en(10'(2 * sx), 10'(2 * sy), THICKNESS_T1);
                    ...
                    if (box_en !== model_box) begin
                        if (mism < 10)
                            $display("        mismatch  T=%0d src(%0d,%0d) exp=%0b act=%0b",
                                     THICKNESS, sx, sy, model_box, box_en);
                        mism = mism + 1;
                    end
                    if (box_en_t1 !== model_box_t1) begin
                        if (mism_t1 < 10)
                            $display("        mismatch  T=%0d src(%0d,%0d) exp=%0b act=%0b",
                                     THICKNESS_T1, sx, sy, model_box_t1, box_en_t1);
                        mism_t1 = mism_t1 + 1;
                    end
                end
            end
    ...
    initial begin
        ...
        // ------------------------------------------------------
        $display("\n--- PHASE 4: full 320x240 source sweep (box0 40x40) ---");
        sweep_frame("P4-BOX0", border_area(40, 40, int'(THICKNESS)),
                    border_area(40, 40, int'(THICKNESS_T1)));
        ...
        $display("\n--- PHASE 5b: full sweep of the moved box (24x30) ---");
        sweep_frame("P5-BOX1", border_area(24, 30, int'(THICKNESS)),
                    border_area(24, 30, int'(THICKNESS_T1)));
