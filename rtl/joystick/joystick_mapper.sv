...
module joystick_mapper(
    ...
    parameter int unsigned X_MIN     = 12'd4;
    parameter int unsigned X_DZ_LOW  = 12'd480;
    parameter int unsigned X_DZ_HIGH = 12'd520;
    parameter int unsigned X_MAX     = 12'd1220;
    
    parameter int unsigned Y_MIN     = 12'd4;
    parameter int unsigned Y_DZ_LOW  = 12'd470;
    parameter int unsigned Y_DZ_HIGH = 12'd510;
    parameter int unsigned Y_MAX     = 12'd1225;
    ...
    localparam int unsigned X_LEFT_GAIN  = ((160 * Q_SCALE) + (X_LEFT_RANGE / 2)) / X_LEFT_RANGE;
    localparam int unsigned X_RIGHT_GAIN = ((160 * Q_SCALE) + (X_RIGHT_RANGE / 2)) / X_RIGHT_RANGE;
    localparam int unsigned Y_LOW_GAIN   = ((120 * Q_SCALE) + (Y_LOW_RANGE / 2)) / Y_LOW_RANGE;
    localparam int unsigned Y_HIGH_GAIN  = ((120 * Q_SCALE) + (Y_HIGH_RANGE / 2)) / Y_HIGH_RANGE;
    ...
    always_comb begin
        ...
        if (i_filtered_x <= X_MIN) begin
            ...
            mapped_x  = -$singed(x_product[20:12]);
            ...
            mapped_x  = $signed(x_product[20:12]);
        end else begin
            mapped_x = 9'sd160;
        end
    end
    ...
    always_comb begin
        ...
        if (i_filtered_y <= Y_MIN) begin
            ...
            mapped_y  = -$signed(y_product[19:12]);
