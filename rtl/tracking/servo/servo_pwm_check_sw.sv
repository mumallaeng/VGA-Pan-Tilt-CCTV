...
module servo_pwm_check_sw (
    ...
    input  logic [15:0] sw,
    output logic        JA4,   // G2, Pan servo signal line
    output logic        JA10,  // G3, Tilt servo signal line
    ...
    always_comb begin  // index x 15, 12 exceeds 180 clamping
        if (sw_ff2[3:0] >= 4'd12) begin
            ...
            pan_angle_next = {4'b0, sw_ff2[3:0]} * 8'd15;
        end

        if (sw_ff2[15:12] >= 4'd12) begin
            tilt_angle_next = 8'd180;
        end else begin
            tilt_angle_next = {4'b0, sw_ff2[15:12]} * 8'd15;
        end
    end
